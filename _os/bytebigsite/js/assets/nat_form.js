        ...
        $.ajax({
            ...
            success: function(data) {
                if (dataType === 'html') {
                    ...
                    $formWrapper.find('#wrapper_purchase_currency_code').attr('style', 'display:none');
                    $formWrapper.find('#wrapper_payout_currency_code').attr('style', 'display:none');
...
        }, 'slow')  
                ...
                $('#wrapper_submit_btn input[type=submit]').show();
                 var div = document.getElementById('buy-now');
    $('html,body').animate({scrollTop: $("#buy-now").offset().top}, 'slow');  
            },
            error: function() {
                showTopMessage({
                    'title': 'Error',
